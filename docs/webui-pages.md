...
4. **BB84 Live** (`/bb84`) — **client-side** Monte-Carlo photon simulation in a **Web Worker** (~70–100M pulses/s), with optional **WASM (Rust, 907 B)**, **WebGL2** and **WebGPU** tiers -- each benchmarked against the Worker for one round and adopted only if it wins, so the badge names the engine that actually ran, real-time QBER chart, key-pool size, photon-frame table, **Eve toggle** + intercept-probability slider, live engine badge
...
10. **PQC Validator** (`/pqc`) — client-side ML-KEM / ML-DSA / SLH-DSA via @noble, cross-checked against liboqs by `POST /api/pqc/interop`. (This said `/api/interop/mlkem`, which is the pqc-validator's INTERNAL route that the backend proxies to -- a reader testing it against the public demo finds no such endpoint)
