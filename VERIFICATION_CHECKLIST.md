...
| 4.6.17 | **Every page that produces evidence can export it** | Systematic sweep of all 13 routes, 2026-08-22: export toolbars on `/`, `/e2e`, `/paper-flow`, `/bb84`, `/pqc`, `/verify`, `/benchmarks`, `/console`. `/topology`, `/vpn`, `/keyflow`, `/hil` are display-only and correctly have none. `/verify` in particular is headed "Implementation Verification" and previously offered no way to export the verification -- a reviewer had to screenshot the table. |
...
| 6.4 | Private files untracked and unreferenced | `git ls-files \| grep -cE 'commercial\|private-notes'` -> 0, and `grep -rl commercial` over tracked files -> empty. Untracked is not enough; a reference from a tracked file discloses the name. |
...
| 6.9b | **Every published formula typesets** | `cd services/webui-frontend && npx vitest run src/lib/docsLatex.test.ts` -> passes. Renders every `$...$` and `$$...$$` in `git ls-files '*.md'` through KaTeX in strict mode. Two equations in `docs/keyrate.md` -- the QBER (Eq. 11) and the GLLP key rate (Eq. 12), the two the golden vector is derived from -- had never rendered on GitHub. Do NOT substitute a brace-counting check: in both defects the braces balanced AND `\left`/`\right` balanced, across the wrong groups, so counting passed them. Fenced blocks and inline code spans are blanked first so `$VAR` in a shell example is not read as maths. |
